...
module AGEX_STAGE(
  ...
  wire [`DBITS-1:0] predicted_pc_AGEX;
  wire [`BHRBITS-1:0] pht_index_AGEX;
  wire pht_valid_AGEX;
  wire instruction_is_ctrl_AGEX;
  wire actual_taken_AGEX;
  wire [`DBITS-1:0] resolved_next_pc_AGEX;
  ...
  wire branch_event_AGEX;
  wire prediction_correct_AGEX;
  reg [63:0] bp_branch_count /* verilator public */;
  reg [63:0] bp_correct_count /* verilator public */;
  ...
  assign instruction_is_ctrl_AGEX = (is_br_AGEX || is_jmp_AGEX);
  assign actual_taken_AGEX = (is_jmp_AGEX) || (is_br_AGEX && br_cond_AGEX);
  assign resolved_next_pc_AGEX = actual_taken_AGEX ? br_target_AGEX : pcplus_AGEX;
  assign br_mispred_AGEX = (valid_AGEX && instruction_is_ctrl_AGEX && (predicted_pc_AGEX != resolved_next_pc_AGEX)) ? 1 : 0;
  assign branch_event_AGEX = valid_AGEX && instruction_is_ctrl_AGEX;
  assign prediction_correct_AGEX = branch_event_AGEX && ~br_mispred_AGEX;
  assign btb_update_en_AGEX = valid_AGEX && instruction_is_ctrl_AGEX;
  assign btb_update_pc_AGEX = PC_AGEX;
  assign btb_update_target_AGEX = br_target_AGEX;
  assign pht_update_en_AGEX = valid_AGEX && instruction_is_ctrl_AGEX && pht_valid_AGEX;
  assign pht_update_taken_AGEX = actual_taken_AGEX;
  assign bhr_update_en_AGEX = valid_AGEX && instruction_is_ctrl_AGEX;
  assign bhr_update_taken_AGEX = actual_taken_AGEX;
  ...
  always @(posedge clk) begin
    if (reset) begin
      bp_branch_count <= 64'd0;
      bp_correct_count <= 64'd0;
    end else if (branch_event_AGEX) begin
      bp_branch_count <= bp_branch_count + 64'd1;
      if (prediction_correct_AGEX)
        bp_correct_count <= bp_correct_count + 64'd1;
    end
  end
