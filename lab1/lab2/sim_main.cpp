...
int main(int argc, char** argv, char** env) {
    ...
    uint64_t total_branches = static_cast<uint64_t>(dut->pipeline->my_AGEX_stage->bp_branch_count);
    uint64_t correct_branches = static_cast<uint64_t>(dut->pipeline->my_AGEX_stage->bp_correct_count);
    double accuracy = (total_branches == 0) ? 0.0 : (static_cast<double>(correct_branches) * 100.0) / static_cast<double>(total_branches);
