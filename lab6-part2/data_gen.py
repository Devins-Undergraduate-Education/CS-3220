...
def generate_random_data_for_4_4_systolic_array(a_size=(4,4),b_size=(4,4), c_size=(4,4), data_bitwdith=3, num_test=4, quiet=False):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--quiet', action='store_true', help="Suppress matrix dumps")
    ...
    if args.mode == "gen_data":
        ...
        generate_random_data_for_4_4_systolic_array(a_size=a_size, b_size=b_size, c_size=c_size, num_test=args.num_tests, quiet=args.quiet)
    else:
        # result = verify_results("c_matrix.bin", "results.bin")
        result = verify_results("d_matrix.bin", "results.bin", row_size=a_size[0], col_size=c_size[1], k_size=a_size[1], num_tests=args.num_tests, quiet=args.quiet)
        if result:
            print("PASSED!")
        else:
            print("FAILED!")
