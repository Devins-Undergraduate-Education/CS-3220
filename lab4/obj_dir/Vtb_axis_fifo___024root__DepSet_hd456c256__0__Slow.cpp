...
#endif
            VL_FATAL_MT("tb_axis_fifo.v", 4, "", "Settle region did not converge.");
        }
        __VstlIterCount = ((IData)(1U) + __VstlIterCount);
        __VstlContinue = 0U;
        if (Vtb_axis_fifo___024root___eval_phase__stl(vlSelf)) {
            __VstlContinue = 1U;
        }
...
VL_ATTR_COLD void Vtb_axis_fifo___024root___dump_triggers__nba(Vtb_axis_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_axis_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_axis_fifo___024root___dump_triggers__nba\n"); );
    // Body
